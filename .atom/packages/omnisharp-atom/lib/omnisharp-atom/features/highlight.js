...
var Highlight = (function () {
    function Highlight() {
        ...
        var loaded = false;
        atom.config.observe("omnisharp-atom.enhancedHighlighting", function (enabled) {
    ...
    Highlight.prototype.activate = function () {
        ...
        this.disposable.add(Omni.editors
            .subscribe(function (editor) { return _this.setupEditor(editor); }));
        ...
        this.disposable.add(isObserveRetokenizing(Omni.listener.observeHighlight
            .map(function (z) { return ({ editor: lodash_1.find(atom.workspace.getTextEditors(), function (editor) { return editor.getPath() == z.request.FileName; }), request: z.request, response: z.response }); })
            .flatMap(function (z) { return Omni.activeEditor.take(1).where(function (x) { return x === z.editor; }).map(function (x) { return z; }); }))
            .debounce(400)
            .subscribe(function (_a) {
            var request = _a.request, response = _a.response, editor = _a.editor;
    ...
    Highlight.prototype.setupEditor = function (editor) {
        var _this = this;
        ...
        if (!editor.displayBuffer.tokenizedBuffer.silentRetokenizeLines) {
            editor.displayBuffer.tokenizedBuffer.silentRetokenizeLines = function () {
        ...
        disposable.add(issueRequest
            ...
            if (!linesToFetch || !linesToFetch.length)
                linesToFetch = [];
            Omni.request(editor, function (client) { return client.highlight(client.makeDataRequest({
...
function isObserveRetokenizing(observable) {
    return observable.where(function (z) { return !!z.editor.getGrammar; }).where(function (z) { return !!z.editor.getGrammar().isObserveRetokenizing; })
        .flatMap(function (z) { return z.editor.getGrammar().isObserveRetokenizing.where(function (z) { return !!z; }).take(1).map(function (x) { return z; }); });
}
function isEditorObserveRetokenizing(observable) {
    return observable.where(function (z) { return !!z.getGrammar; }).where(function (z) { return !!z.getGrammar().isObserveRetokenizing; })
        .flatMap(function (z) { return z.getGrammar().isObserveRetokenizing.where(function (z) { return !!z; }).take(1).map(function (x) { return z; }); });
}
...
exports.highlight = new Highlight;
